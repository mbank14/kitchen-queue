...
export const OrderIn = ({orderData}: {orderData:IOrderIn[]}) => {
            ...
            <div className={`w-full overflow-auto`}>
                ...
                <div className={`flex`}>
                    {orderData.map((item, index) => (
                        <div>
                        <div key={index} className={`${item.status === 'pending' ? 'bg-amber-400' : ''}
                           ...
                           <div className={`p-3 `}>
                                ...
                                <div className={`flex flex-row gap-1 mt-5`}>
                                    {/* {item.status === 'pending' ? <span>Pending</span> : 
                                        <button onClick={() => (item.no)} className={`bg-amber-400 w-1/2 py-2 rounded-xl`}>Pending</button>
                                    } */}
                                    <button className={`bg-sky-400 w-1/2 py-2 rounded-xl`}>Process</button>
                                </div>
                           </div>
                        </div>
                        </div>
